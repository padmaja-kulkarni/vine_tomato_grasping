...
import json
import numpy as np
from utils.util import load_rgb, plot_features, plot_error, make_dirs, plot_features_result
from detect_truss.detect_tomato import compute_com
...
for count, i_truss in enumerate(range(i_start, i_end)):
    ...
    com_error = euclidean_dist(tomato_lbl['com'][0], tomato_res['com']) / px_per_mm

    # compute error
    tomato_error = {'radii': [], 'centers': [], 'com': com_error, 'n_true_pos': n_true_pos, 'n_false_pos': n_false_pos,
                    'n_labeled_pos': n_labeled_pos, 'n_predict_pos': n_predict_pos}
    ...
    junctions_error = {'centers': [], 'true_pos': n_true_pos, 'false_pos': n_false_pos, 'labeled_pos': n_labeled_pos,
