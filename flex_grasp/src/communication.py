...
class Communication(object):
    ...
    def wait_for_result(self, command):
        ...
        while (curr_time - start_time < self.timeout) and not rospy.is_shutdown():
            ...
            if self.result is not None:
                ...
                if self.result == FlexGraspErrorCodes.SUCCESS:
                    rospy.logdebug("Command succeeded: received success on topic %s", self.topic)
                    return self.result
                else:
                    rospy.logdebug("Command failed: node returned %s on topic %s", self.result, self.topic)
                    return self.result   
        ...
        rospy.logwarn("Command failed: node did not return a message within timeout on topic %s", self.topic)
