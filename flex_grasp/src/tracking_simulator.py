...
def parse_transformation(t_str):
    t_str_split = t_str.split(' ')
    length = len(t_str_split)

    if length==6:
        tx, ty, tz, rx, ry, rz = [float(t) for t in t_str_split[0:6]]
        quat = np.array(quaternion_from_euler(rx, ry, rz))

    elif length ==7:
        tx, ty, tz, rx, ry, rz, rw = [float(t) for t in t_str_split[0:7]]
        quat = np.array([rx, ry, rz, rw])

    else:
        print("transformation should be expresessed as [position, euler] or [position, quaternion]")

    return np.array([tx, ty, tz]), quat
